...
-   They are responsible for starting the database nodes and supervising them to ensure they are restarted in case of a crash or termination owing to abnormal circumstances. This is known as the _watchdog_ service.
-   They communicate with other manager nodes to determine cluster state, and relay that information to the database nodes under management.
...
You can determine the configuration of a node by passing the `-WhatIf` flag to the process.
...
The watchdog configuration file details which database nodes the manager is responsible for starting and supervising. Unlike the other configuration files, the manager configuration uses a custom format instead of YAML. Each node for which the manager is responsible has one line in the file, which starts with a `#` symbol and then details the command line options given to the database node when it is started. Under normal circumstances, this will be the path to the database node’s configuration file.
...
In this case, Event Store is deployed on each node in \_c:\\EventStore-HA-v3.0.1\_, and the configuration files for that node are deployed into \_C:\\EventStore-Config\_. No installation process is necessary, the packaged distribution can be unzipped, assuming they have been unblocked following download.
...
```powershell
C:\EventStore-HA-v3.0.1\> EventStore.WindowsManager.exe install -InstanceName es-cluster1 -ManagerConfig
