...
There is a special paged feed for all events named `$all`. You can use the same paged form of reading described above to read all events for a node by pointing the stream at _/streams/$all_. As it's a stream like any other, you can perform all operations, except posting to it.

> [!NOTE]
> To access the `$all` stream, you must use user details, find more information on the [security](~/http-api/security.md) page.
