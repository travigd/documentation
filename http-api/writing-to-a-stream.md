...
The event will be available in the stream. Some clients may not be able to generate a GUID (or may not want to generate a GUID) for the ID. You need this ID for idempotence <!-- is this a real word? --> purposes but the server can generate it for you. If we were to leave off the `ES-EventId` header you would see different behavior:
...
The data can be represented by the following jschema (`eventId` must be a UUID).
...
<div data-lang="request" markdown="1">
```bash
curl -i -d @myevent.txt "http://127.0.0.1:2113/streams/newstream" -H "Content-Type:application/vnd.eventstore.events+json"
```
</div>
<div data-lang="response" markdown="1">
```http
...
<!-- Is this test.js the same as the txt? -->

<div class="codetabs" markdown="1">
<div data-lang="request" markdown="1">
```bash
curl -i -d @test.js "http://127.0.0.1:2113/streams/newstream" -H "Content-Type:application/json" -H "ES-ExpectedVersion: 3"
```
</div>
<div data-lang="response" markdown="1">
```http
...
This is important behavior as this is how error handling works <!-- in event store? -->. If you receive a timeout, broken connection, no answer, etc from your HTTP `POST` then it's your responsibility is to retry the post. You must also keep the same `uuid` that you assigned to the event in the first `POST`.

If you are using the expected version parameter with your message, then Event Store is 100% idempotent. If you use `Any` <!-- Is this a parameter? --> as your expected version,  Event Store will do its best to keep events idempotent but cannot assure that everything is fully idempotent and you will end up in 'atleast-once' messaging <!-- Is this correct? -->. This is discussed further in the idempotency document.
...
<div data-lang="request" markdown="1">
```bash
curl -i -d @myevent.json "http://127.0.0.1:2113/streams/newstream" -H "Content-Type:application/json" -H "ES-EventType: SomeEvent"
```
</div>
<div data-lang="response" markdown="1">
```http
...
<div data-lang="request" markdown="1">
```bash
curl -i -d @myevent.json "http://127.0.0.1:2113/streams/newstream/incoming/c7248fc1-3db4-42c1-96aa-a071c92649d1" -H "Content-Type: application/json" -H "ES-EventType: SomeEvent"
```
</div>
<div data-lang="response" markdown="1">
```http
